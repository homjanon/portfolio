...
| 三市场均休市（通常周日/周一） | **精简模式** | 仅抓 `data_news.json`（Top20：谷歌美国20条→LLM精选≤10仅谷歌来源不补位 + 联合早报最新10,统一六实例兜底；两块独立互不补位、不强制凑满20）+ `data_deep.json`（深度观察专栏·**双源**：联合早报 `/zaobao/other/forum/views` + 法广中文 `/rfi/cn`，各取最新 5 条；**先按关键词硬筛「非中美」与非文章条目，再由 LLM 选 1 篇中国/美国之外的第三方深度文章原文直出**；双源均无合格候选则今日暂停） |
...
| **深度观察专栏（仅精简模式）** | **双源**：**《联合早报》时事与新闻评论** `/zaobao/other/forum/views` + **法广中文** `/rfi/cn`（统一实例池兜底，但为深度源启用 **desc 中位长度门槛 ≥700 字**：不达标继续试下一实例、全不达标取最长者并告警——根治“锁死导语版实例”；每源取最新 5 条，池 ≤10 条）。**代码侧先硬筛**：标题含涉中美词（中国/美国/中美/台海/港澳/新疆/西藏/南海/白宫/中南海/习近平/特朗普…含「对美」「两强相争」等隐性词）或非文章条目（播音节目表）一律剔除，超长文（>6000 字）不入池；**再由 LLM 做语义精筛并选 1 篇「中国与美国之外」的第三方深度文章原文直出**（零改写；LLM 须排除标题干净但主题涉中美的条目，如《AI两强相争，各国如何因应？》；合规候选均不够深度时降级选其中话题性最强的一篇；全部涉中美则今日暂停） | 仅精简模式 | `data_deep.json`（`items_deep` 数组） |
...
> ⚠️ **深度观察专栏是例外**：2026-09-21 实测发现各实例返回的 `desc` 深浅不一——同一篇文章 `hub.slarker.me` 只给导语（中位 298 字，约为全文 23%），`rsshub.umzzz.com` 给全文（中位 1844 字），isrss/ktachibana/virworks 与 slarker 同为导语版。若沿用「命中即止」，专栏会常年只输出文章开头（线上 9/21 仅 334 字、9/20 仅 511 字）→ **深度源改用独立实例顺序（`rsshub.umzzz.com` 首选）+ desc 中位 ≥700 字门槛**。
