...
| **市场全景各板块一句话简述** | **财联社 RSS 双源（主 `rsshub.rssforever/cls/telegraph` + 兜底 `hub.slarker.me/cls/depth/1000`，合并去重+当天筛选；LLM 自行提炼 A股/港股/美股/全球/大宗 各一句）** | 完整模式 | 无当天新闻则留空（不编造） |
...
`scripts/md_to_script.py` 将 `report.md` 转为口语化广播稿 `script.txt`（约6分钟，1200-1600字），复用 `call_llm.py` 的 `LLM_CONFIGS` 与 `_call_llm`（单一数据源）：
...
├── scripts/
│   ├── prefetch_data.py                     # 数据抓取（市场全景+估值+QDII/ETF+新闻；新闻：Google News 美国单地30条→去重,LLM选≤10 + 联合早报按缺口补齐至20 双源 Top20；data_deep.json 取联合早报>700字长文供深度观察专栏；data_cls_zaobao.json 取财联社 RSS 双源(主 rsshub.rssforever/cls/telegraph + 兜底 hub.slarker.me/cls/depth/1000)当天新闻供市场全景各板块一句话简述+持仓聚焦）；已停抓 data_fund/data_industry/data_holdings（LLM 输入 JSON 由 11→8）
