...
v36: 估值段蛋卷targets定为11指数(删恒生指数/恒生国企/中证1000, 加中证白酒/中概互联50, 均来自蛋卷63返回零额外网络)；估值快照改固定顺序遍历；重写_shorten_qdii_name(份额字母锚定末尾/X类, 修复建信C人纳指100→建信纳指100c)；QDII对比昨日+名称_短保持
...
# ═══════════════════════════════════════════════════════════════
def main():
    print(f"═══ 预抓取金融市场数据（v36: 估值段蛋卷targets定为11指数(删恒指/国企/中证1000,加中证白酒/中概互联50) | 估值快照固定顺序 | QDII简称修复(建信C人→建信纳指100c) | QDII对比昨日+名称_短保持 | 东财push2主源secid修正+yfinance兜底） ═══")
    print(f"时间: {_ts()}\n")
